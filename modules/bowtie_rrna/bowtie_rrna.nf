...
process bowtie_rrna {
    ...
    output:
        ...
        path "${sample_id}_bowtie.log", emit: report

    shell:
    //bowtie [options]* <ebwt> {-1 <m1> -2 <m2> | --12 <r> | --interleaved <i> | <s>} [<hit>]
    """
    gunzip -c $reads | \
    bowtie -v 2 -m 1 --best --strata --threads ${task.cpus} -q --sam --norc --un ${sample_id}.unaligned.fq ${bowtie_index}/small_rna_bowtie - 2> ${sample_id}_bowtie.log | \
